...
public class TestFetchWeatherTask extends AndroidTestCase {
  ...
  static final double ADD_LOCATION_LON = -119.714167;
    ...
    /*
        ...
        This test will only run on API level 11 and higher because of a
        requirement in the
        content provider.
     */
  //    @TargetApi(11)
  ...
  //        getContext().getContentResolver().delete(WeatherContract
  ...
  //                assertEquals("Error: the queried value of locationId does
  // not match the returned value" +
  //                        "from addLocation", locationCursor.getLong(0),
  // locationId);
  //                assertEquals("Error: the queried value of location
  // setting is incorrect",
  //                        locationCursor.getString(1), ADD_LOCATION_SETTING);
  //                assertEquals("Error: the queried value of location city
  // is incorrect",
  //                        locationCursor.getString(2), ADD_LOCATION_CITY);
  //                assertEquals("Error: the queried value of latitude is
  // incorrect",
  //                        locationCursor.getDouble(3), ADD_LOCATION_LAT);
  //                assertEquals("Error: the queried value of longitude is
  // incorrect",
  //                        locationCursor.getDouble(4), ADD_LOCATION_LON);
  //            } else {
  //                fail("Error: the id you used to query returned an empty
  // cursor");
  //            }
  //
  //            // there should be no more records
  //            assertFalse("Error: there should be only one record returned
  // from a location query",
  //                    locationCursor.moveToNext());
  //
  //            // add the location again
  //            long newLocationId = fwt.addLocation(ADD_LOCATION_SETTING,
  // ADD_LOCATION_CITY,
  //                    ADD_LOCATION_LAT, ADD_LOCATION_LON);
  //
  //            assertEquals("Error: inserting a location again should return
  // the same ID",
  ...
  //                new String[]{ADD_LOCATION_SETTING});
  //
  //        // clean up the test so that other tests can use the content
  // provider
  //        getContext().getContentResolver().
  //                acquireContentProviderClient(WeatherContract
  // .LocationEntry.CONTENT_URI).
